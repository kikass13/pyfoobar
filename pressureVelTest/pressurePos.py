...
def main():
    ...
    pidParams = pid.Parameter(Kp=Kp, Ki=Ki, Kd=Kd, Ts=0.05, limitMin=4000.0, limitMax=10000.0)
    ...
    while T >= time:
        ...
        elif time < 0.3 and time > 0.1:
            target_pressure = 80.0
        elif time > 0.3 and time < 0.7:
        ...
        if target_pressure <= 25.0:
            positionState = 0.0
            controller.resetIntegral()
        else:
            error = target_pressure - currentPressure
            positionState = controller(error, dt)

        # newPos = controller(error, dt)
        # if newPos < 4000:
        #     positionState = 0.0
        #     # controller.resetIntegral()
        # else:
